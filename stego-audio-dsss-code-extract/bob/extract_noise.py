...
def dsss_extract(stego_file, param_file='embed_params.npz'):
    ...
    # Thêm nhiễu trắng
    noise_amplitude = 0.05  # Biên độ nhiễu, có thể thay đổi
    noise = np.random.normal(0, noise_amplitude, len(audio_data))
    audio_data += noise
    audio_data = audio_data / np.max(np.abs(audio_data))  # Chuẩn hóa lại
    ...
    try:
        with open('extracted_message_noise.txt', 'w', encoding='utf-8') as f:
            f.write(message)
        print(f"✅ Đã lưu thông điệp vào 'extracted_message_noise.txt'.")
